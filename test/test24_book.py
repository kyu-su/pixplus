...
class Test_Manga(TestCase):
  ...
  def check(self, name, page, image_numbers):
    ...
    diff = ImageChops.difference(got, expected)
    got.save('test24_book_%s_%s_got.png' % (name, ','.join(map(str, image_numbers))))
    expected.save('test24_book_%s_%s_expected.png' % (name, ','.join(map(str, image_numbers))))
    diff.save('test24_book_%s_%s_diff.png' % (name, ','.join(map(str, image_numbers))))
